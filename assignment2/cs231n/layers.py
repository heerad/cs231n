...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        # variables.                                                          #
        #######################################################################
        # compute mean and variance on minibatch
        mb_mean = np.mean(x,axis=0)
        mb_var = np.mean((x - mb_mean)**2,axis=0)
        
        # normalize using this minibatch's mean/variance as well as epsilon
        x_hat = (x - mb_mean) / np.sqrt(mb_var + eps)
        
        # scale and shift using gamma and beta
        out = gamma*x_hat + beta
        
        # update running EMA of mean and variance
        running_mean = momentum * running_mean + (1 - momentum) * mb_mean
        running_var = momentum * running_var + (1 - momentum) * mb_var
        
        cache = (x, gamma, x_hat, mb_mean, mb_var, eps)
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    x, gamma, x_normalized, sample_mean, sample_var, eps = cache
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    x_pad = np.pad(np.copy(x), ((0,0),(0,0),(pad,pad),(pad,pad)), 'constant', constant_values=0)
...
def conv_backward_naive(dout, cache):
    ...
    dx = dx[:,:,pad:(x_pad.shape[2]-pad),pad:(x_pad.shape[3]-pad)]
...
def max_pool_forward_naive(x, pool_param):
    ...
    for n in range(N):
        for c in range(C):
            for i in range(H2):
                for j in range(W2):
                    out[n,c,i,j] = np.max(x[n,c,(stride*i):(stride*i+ph),(stride*j):(stride*j+pw)])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
        ...
        for c in range(C):
            for i in range(H2):
                for j in range(W2):
                    ...
                    for k in range(stride*i,stride*i+ph):
                        for l in range(stride*j,stride*j+pw):
                            current = x[n,c,k,l]
                            if current > best:
                                best_k = k
                                best_l = l
                    ...
                    dx[n,c,best_k,best_l] += dout[n,c,i,j]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
